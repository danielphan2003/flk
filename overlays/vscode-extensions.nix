final: prev: {
  vscode-extensions = with prev; vscode-extensions // {
    # ms-vscode = vscode-extensions.ms-vscode // {
    #   cpptools = let
    #     inherit (final.sources.vscode-extensions-ms-vscode-cpptools) src version;
    #   in vscode-extensions.ms-vscode.cpptools.override {
    #     vscode-utils = vscode-utils // {
    #       buildVscodeMarketplaceExtension = args:
    #         vscode-utils.buildVscodeMarketplaceExtension (args // {
    #           vsix = with args.mktplcRef; "${publisher}-${name}.zip";
    #           preUnpack = ''ln -s "${src}" $src'';
    #           mktplcRef = args.mktplcRef // { inherit version; };
    #           prePatch = ''
    #             ln -s ./debugAdapters/bin/* ./debugAdapters
    #           '';
    #         });
    #     };
    #   };
