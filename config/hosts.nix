...
with lib; let
  ...
  hostOpts = types.submodule ({name, ...} @ args: {
    options = singleHostOpts;

    config = {
      name = mkDefault name;

      domain = mkDefault args.config.name;

      tailscale = {
        magicdns.machineName = mkDefault args.config.name;
        https.machineName = mkDefault args.config.name;
      };
    };
  });
