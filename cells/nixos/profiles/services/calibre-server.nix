...
in {
  ...
  services.calibre-server = {
    enable = true;
    libraries = ["${home}/${config.networking.hostName}"];
  };
  ...
  services.caddy.virtualHosts.calibre-server = {
    hostName = "calibre.${config.networking.domain}";
    extraConfig = ''
      import common
      import NoSearchHeader
      import useCloudflare
      reverse_proxy ${tailscale.ip}:${toString port}
    '';
  };
}
