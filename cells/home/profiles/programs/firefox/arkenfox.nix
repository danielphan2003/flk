{
  ...
  inherit (lib) major mkDefault mkMerge;
  ...
  firefoxVersion = "${major config.programs.firefox.package.version}.0";
...
in {
  programs.firefox = {
    ...
    arkenfoxVersion = mkDefault defaultArkenfoxVersion;
  };
  ...
  in
    mkMerge ([arkenfox.default] ++ mapEnabled arkenfox.enabled ++ mapDisabled arkenfox.disabled);
}
