  ...
  wayland.windowManager.hyprland = {
    ...
    in
      concatNewline [
        ...
        (applyWorkspaceBinds (bind: "bind=SUPER,${toString bind.key},workspace,${concatComma bind.args}"))

        # Binds - move to workspace (silent)
        (applyWorkspaceBinds (bind: "bind=SUPERALT,${toString bind.key},movetoworkspacesilent,${concatComma bind.args}"))
        ...
        # (float {title = "Welcome.*";})
        # (float {app_id = "ulauncher";})
        # (float {class = "onedrive_tray";})
        # (float {class = "Tk";})
        # (float {app_id = "Yad";})
        # (float {app_id = "yad";})
